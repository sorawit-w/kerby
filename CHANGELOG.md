...
**swe 2.11.4 — the provenance guard was not portable, and the platform hid it.** Three defects found by the GitHub-side Codex review, which runs on Linux. **Two of the three only misbehave off macOS**, and the local review that cleared 9.26.3 runs on macOS, so it structurally could not see those. The third — a CSS colour read as a commit SHA — reproduces anywhere and was simply missed. kerby ships to other people's machines, so "passes here" was never the claim worth making.
