...
- **The third trigger is not decoration.** Slot-truth tests whether your *declaration* is accurate, not whether the *work* is still what was approved. A rate-limit constant changed inside an already-named file, an approach swapped in place, a step that became irreversible, a re-grade into the Critical row — all leave every slot true. The loose "shape moves" rule covered these by accident; narrowing without naming them would have traded a fatigue bug for a coverage hole.
