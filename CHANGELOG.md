...
- **`hooks/status-provenance-check.sh` runs the guard on both copies, every time.** The staged blob and the working-tree file, whichever differ from HEAD — type changes included, a symlink as its target text. It does not work out which copy a given `git commit` records. The first cut did: it tokenized the command, resolved pathspecs through `git ls-files`, modelled `-a`/`-i`/`-p`/`--only`/`--dry-run`, stripped redirections — and nine independent review rounds found a hole in every model of the shell it tried: key order, attached separators, quoted and escaped operators, braces, globs in option values, `{fd}>` redirections, abbreviated long options. Every hole was a miss, so the classifier is gone and the hook cannot be talked out of a scan by any shell shape. The cost is a block on a STATUS.md that carries a token in a copy this commit would not have recorded — a state the rule forbids anyway, and the message names the copy. Block severity, `recommended` tier, not disablable; a visible fail-open when its own guard script is missing (the launcher's doctrine); `^git commit` recognition only, the hollow-test ceiling, declared in the manifest `gap`; 40 assertions in a real `git init` fixture, including every command shape that once fooled the classifier. No `git_hook` yet: install Phase 3 writes one pre-commit file that runs one script, and chaining is logged as debt.
