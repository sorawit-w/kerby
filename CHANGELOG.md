...
- **A heartbeat, first line of every session that registers the SessionStart hook.** `session-start-context.sh` now opens with `kerby engine <version> at <root> — hooks via launcher: ok`, or names the problem: launcher missing, launcher outdated, pointer naming a different copy. The root is the script's own location — the copy actually executing — so the line can diagnose a stale pointer rather than trust it.
