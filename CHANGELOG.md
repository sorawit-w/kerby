...
- **The absence is guarded, and the guard ships with its test.** `scripts/check-status-provenance.sh` asserts the file states no semver, no commit-SHA-shaped token, no branch name. Deletion is what makes this checkable: an absence matches *shapes*, not phrasings, so unlike the commit-time gate rule — where three guards were each defeated by ordinary authoring wording — there is nothing a status-file author writes without thinking that slips past it. `check-status-provenance.test.sh` covers all of it in the same commit, including a false-positive probe, so tightening the patterns into something that fires on ordinary prose gets caught rather than deleted by the first person it annoys.
