...
**swe 2.12.0 — a case table in every plan, and docs you read this session.** Two gaps from a rule audit against the shipped rulebook. Expected Outcomes predicted one end-state per medium — the shape of the result — but never enumerated cases, so a plan could be "right" with the edge and failure paths unexamined until a tester found them. And the rule that third-party behavior comes from documentation, not recollection, lived only in a reference file that loads on demand and named no way to fetch the documentation.

- **The prediction gains its tester's half.** At or above `plan_threshold`, the Expected Outcomes block carries a case table — `case | action / input | expected` — with at least the happy path plus one edge case and one failure case where the change has them, and an explicit `none` row where it has none. A forced artifact, not advice: an absent table is a missing plan. The finish step fills an `actual` column row by row from real runs and emits `outcome:` per case; any mismatched case takes the existing routing. At the High/Critical tiers the table is the QA sub-agent's Stage 1 checklist, and the sub-agent re-derives each `actual` itself rather than trusting the implementer's lines. One artifact, two readers, no second format.
- **§ Accuracy reaches past the repo.** It already forbade referencing a file you have not read. It now says the same of a framework or API: behavior a change relies on comes from documentation read this session — through an installed docs-retrieval tool when one exists, else the official docs — because training data is a recollection, not a citation. The full rule stays where it was, in `working-patterns.md` § Source-Driven Framework Claims; the eager clause is one sentence.
