...
- **The absence is guarded, and the guard ships with its test.** `scripts/check-status-provenance.sh` asserts the file states no semver, no SHA-shaped hex token, and no `<type>/<name>` branch reference for any of the seven documented branch types. Deletion is what makes this checkable: an absence matches *shapes*, not phrasings, so unlike the commit-time gate rule — where three guards were each defeated by ordinary authoring wording — the ordinary ways of writing these facts are caught. It is not exhaustive, and the script's header names every gap it knows about: a bare `main`, a hex abbreviation containing no digit, a four-part version. `check-status-provenance.test.sh` covers all of it in the same commit, including a false-positive probe, so tightening the patterns into something that fires on ordinary prose gets caught rather than deleted by the first person it annoys.
