...
- **`quick-task` gains no second artifact.** Its fit check already declares files and type; it now declares the check too and *is* the plan on that route. Escalation voids it, which is the case worth naming: § 3a fires after code exists, so the artifact standing in for the plan is the one that just failed — re-emit the grade and write the full plan before continuing.
