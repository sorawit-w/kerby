...
- **`hooks/status-provenance-check.sh` runs the guard on the file the commit records.** The staged blob, because a clean working tree over a dirty index is exactly what a working-tree scan would pass; the working-tree file when `-a`, `--include` or a pathspec bypasses the index. Block severity, `recommended` tier, not disablable — decline it at `kerby install` instead. It fails open, visibly, when its own guard script is missing (the launcher's doctrine), and it recognises `git commit` at the start of the command only, the same ceiling as the hollow-test advisory; both ceilings are declared in the manifest's `gap`. Twelve cases in its self-test run inside a real `git init` fixture, including `-am` over a dirty tree and a dirty index under a clean tree. No `git_hook` yet: install Phase 3 writes one pre-commit file that runs one script, and chaining is logged as debt.
