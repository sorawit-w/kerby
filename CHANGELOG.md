...
- **Two new shapes, and the PR number leaves STATUS.md.** `#54` and `PR 7552` are tracker identity, and the tracker owns it: name the change in words in the status file, keep the number in the commit message and `memory.log`, where history lives. The shapes are tested on the raw line before the colour blanking, because `#1234` and `#123456` are exactly what blanking removes; a `#` followed by digits only reads as an issue reference even at six digits, the mirror of the 8-digit RGBA trade. `APR 2026`, `PRs`, a markdown heading and a hyphenated `PR-7552` do not match, and each positive case in the test carries the negative twin a careless widening would swallow. This repo's own STATUS.md rows that cited PR numbers are reworded in the same change.
- **A review or merge state is provenance too, and the audit found it.** The split-role audit of this change passed every assertion and then tripped a probe: asked to update STATUS.md after a PR opened, the executor kept the number out and wrote `PR open, awaiting review/merge` into the Phase cell anyway — the sibling-repo failure in miniature, and one no token guard can match. § Status Tracking now names those phrasings and the shape that replaces them (`Review — rate-limit fix`); the re-run wrote the phase and nothing about the pull request.
- **`hooks/status-provenance-check.sh` runs the guard on the file the commit records.** The staged blob, because a clean working tree over a dirty index is exactly what a working-tree scan would pass; the working tree under `-a`; and for a pathspec commit the working tree of the named paths only, with the pathspecs *resolved* through `git ls-files` rather than text-matched — `.`, `.kerby`, `:/` and a relative path from a subdirectory all reach the file, `--include other` still records a staged one, `-p`/`--interactive` (worktree hunks over the index) and anything undecidable — an unbalanced quote, a heredoc, a command substitution — scan both sources, a shell redirection is never a pathspec, `--pathspec-from-file` entries count as pathspecs, and `--no-all` negates like git does. The classifier's default is the block: an option it does not list (an abbreviation like `--inc`, an unknown short letter, a flag git adds later), a variable, glob or tilde in a pathspec, a quoted line in a pathspec file, an attached separator's remainder — anything it cannot prove — scans both sources; a staged type change is scanned like any other blob; `--only --amend` with no pathspec records HEAD's tree and is let through. The independent review reproduced the text-match miss on the first cut and thirteen more command shapes over three further rounds, which is why the default flipped from precision to the block; the self-test grew from twelve to 66 assertions; `--dry-run` is let through. Block severity, `recommended` tier, not disablable — decline it at `kerby install` instead. It fails open, visibly, when its own guard script is missing (the launcher's doctrine), and it recognises `git commit` at the start of the command only, the same ceiling as the hollow-test advisory; both ceilings are declared in the manifest's `gap`. Its self-test runs inside a real `git init` fixture, including `-am` over a dirty tree, a dirty index under a clean tree, and the resolved-pathspec cases. No `git_hook` yet: install Phase 3 writes one pre-commit file that runs one script, and chaining is logged as debt.
- **No footprint.** Commit messages, PR titles and PR bodies describe the change and carry no attribution to the tool, model or vendor that produced them — no agent `Co-authored-by` trailer, no "generated with" line — unless the project's own instructions ask for one. The rule says in so many words that it overrides a harness-supplied attribution reminder, because that reminder is exactly what put the footprint there; a project that wants a trailer for audit keeps it by asking, and the audit's project-override case confirmed that path lands. One clause in BOOTSTRAP § Commit Discipline sits at the decision point; the paragraph lives in § PR Title & Body. The root `CLAUDE.md` § PR Workflow carries the same line for this repo.
- **Audited.** Split-role, fresh-context executors and graders, with the test set designed by a subagent that had not seen the author's reasoning: 26/26 on the first pass, the probe finding above fixed in rule text, 5/5 on the re-run — 2026-09-16. Codex review on the pull request.

## [10.1.0] — 2026-09-16

**The rules come back after a compaction, and nobody has to remember to ask.** `kerby load` reads rule prose into context through the Read tool, which makes the rules a tool result — and a tool result is exactly what compaction summarizes. After a compaction the pin still said loaded, the confirmation line still sat in the summary, and the rules were gone. A sibling repo found out the hard way: an agent wrote a pull-request number, "awaiting review" and a SHA into `.kerby/STATUS.md` with two rules against it in force, both of them out of context. `reload` was always the remedy; it depended on the agent noticing, which is the one thing a compacted agent cannot do.

- **The SessionStart hook re-injects the rules on `source: "compact"`.** Claude Code fires SessionStart after a compaction with that source and adds the hook's stdout to context, so `session-start-context.sh` now reads the payload and, on `compact`, prints every pinned builtin's eager prose — the set `load` step 4 reads (root body, `floor = true`, `token_cost = "low"`), derived from each manifest by the hook itself, never restated. For the three bundled builtins that is 62,000 bytes, about the cost of a `reload`, spent only when a compaction has just freed the space. `fork` inherits the parent's tool results and `resume` restores the transcript, so neither triggers it.
- **The workspace steers nothing but *which* builtins.** Only the lock's `selected` ids are read; only an entry the lock marks `builtin` counts, and only the validator's slug shape is ever used as a path; every path resolves under the hook's own install root, never `path_or_url` and never the pointer. A body path that leaves its folder is refused. An external rulebook gets one nudge line and no text — admitting its prose is the trust prompt's job, and only `load`/`reload` run it. The rule text prints as install-trusted rules, not `DATA>`; the state blocks below it keep their prefix.
- **The install line and the compaction caveat now say so.** Both name the re-injection and keep `reload` as the manual path. It is Claude Code-only — SessionStart is its event; Codex never runs hooks, Copilot is unverified — and only fires when the SessionStart service is registered, which `install` offers as an `optional` row. The confirmation strings are unchanged; `docs/ENGINE-MAP-v2.md`'s parity row records that the caveat text moved and the confirmations did not.
- **Tested in the same commit.** `session-start-context.test.sh` gains the compaction cases: the four other sources print nothing; `compact` prints exactly the eager set in load order, floor first, un-prefixed, with the state echo still framed; an external id and a hostile id are each handled without printing text; no lock nudges `load`; a delete-swe drill against a reduced install copy exits 0 with a "does not ship" line; an escaping body path is refused. Every existing invocation now runs with stdin closed, so the new read can never block a harness.

## [10.0.0] — 2026-09-05

**The `codex-review` rulebook is retired.** It shipped a review-until-clean loop, and that loop cannot converge: a model review is a sampler, not a proof, so a second pass over identical bytes finds new things — and did. The audit log says it plainly: nineteen passes, a median of one round but seven runs at the three-round cap; eleven failed attempts, seven of them stalls, about seventy-six minutes lost to a gate that could not decide. The GitHub-side `@codex review` found new defects after a clean local pass anyway (9.26.4 records three), so the local loop duplicated it without replacing it. The wrapper needed six hardening PRs, most about process-group kill semantics — a mechanism fighting its environment. And a review → fix → re-review sequence with a cap and a judge is a *workflow*: laney's shape, not a rulebook's. The review stays — it is the `@codex review` on the pull request — and its future home is a laney workflow. **Breaking** — read the migration notes:

- **⚠️ A pin that still selects `codex-review` fails closed.** The id no longer ships, so a `selected` entry naming it takes the ordinary unknown-builtin HELD on the next `load` — no auto-migration, no silent drop. Fix, per repo: `kerby unload codex-review`, then `kerby install` — the stale-entry prune (install § 5) clears the dead `codex-pr-gate.sh` registration from your settings file; `status` flags it `registered script missing` until you do. A committed `.kerby/rulebooks.toml` naming it needs the entry removed by hand. Update the installed plugin *first*, or the builtin still exists on your machine and nothing is stale yet.
- **⚠️ The v9.0.0 `code` → `swe` migration residue is gone, on schedule.** SKILL.md's marked section and every sentence that pointed at it are deleted; `docs/rulebook-contract.md` no longer exempts it from the zoning rule. A pre-v9 pin naming `code` now takes the same unknown-builtin HELD; the fix has the same shape — `kerby unload code`, `kerby load swe`, `kerby install`.
- **Three builtins, not four.** `base`, `swe`, `skill-authoring`. Every engine surface that named the fourth — the builtin list, the additive-load worked example, the hook-tier reference, the contract's `override` example, the disable-tier guard's comments and its test (eleven scripts classified, not twelve) — names a survivor or nothing. The delete-the-rulebook drill from the contract's § Engine independence ran for real: `grep codex-review` across engine, docs, scripts, and rulebooks returns nothing.
- **The PR workflow has one path.** Root `CLAUDE.md` § PR Workflow: open the PR, trigger `@codex review`, address every comment against the current head, merge on a green light. The local path, the marker, the round cap, and the `gh pr create` gate left with the rulebook; `skills/kerby/CLAUDE.md`'s gate row says "on the PR" and nothing else. The STATUS.md note that settled "a clean local review is sufficient to merge" is rewritten — that settlement was the defect.
- **Recorded, not just removed.** `.kerby/knowledge/decision-remove-codex-review.md` carries the numbers and the reasoning; `CONTEXT.md` § Superseded terms maps the retired term to its replacement. Git history keeps the fourteen files (last touched at `3c9ef55`) for the laney seed — the `CODEX_VERDICT` grammar and P0/P1 triage in `references/pr-workflow.md` and `scripts/codex-mark.sh`.

## [9.28.0] — 2026-09-04

**swe 2.12.0 — a case table in every threshold-gated plan, and docs you read this session.** Two gaps from a rule audit against the shipped rulebook. Expected Outcomes predicted one end-state per medium — the shape of the result — but never enumerated cases, so a plan could be "right" with the edge and failure paths unexamined until a tester found them. And the rule that third-party behavior comes from documentation, not recollection, lived only in a reference file that loads on demand and named no way to fetch the documentation.

- **The prediction gains its tester's half.** At or above `plan_threshold`, the Expected Outcomes block carries a case table — `case | action / input | expected` — with the happy path always, and an edge and a failure row each where the change has one — or a `none — <why>` row for the category that has none. A forced artifact, not advice: an absent table is a missing plan. The finish step fills an `actual` column row by row from real runs and emits one `outcome:` line per row; a row whose condition never arose reads `not exercised`, never `match`, because a match is a comparison and a case that never ran has nothing to compare. Any mismatched case takes the existing routing. At the High/Critical tiers the table is the QA sub-agent's Stage 1 checklist, and the sub-agent re-derives each `actual` itself rather than trusting the implementer's lines. One artifact, two readers, no second format.
- **§ Accuracy reaches past the repo.** It already forbade referencing a file you have not read. It now says the same of a framework or API: behavior a change relies on comes from something read this session — the documentation through an installed docs-retrieval tool, else the official docs, else the installed package's own source or type definitions — because training data is a recollection, not a citation. Source counts, deliberately: a session with no network and no retrieval tool still has `node_modules` to read, and reading it is what the rule asks for. The full rule stays where it was, in `working-patterns.md` § Source-Driven Framework Claims; the eager clause is one sentence.
- **context7 joins the registry.** `external-resources.md` § MCP Servers gains a row for the docs-retrieval MCP, listed explicitly for the same reason JetBrains MCP is — a rule now names the tool class, so consumers need a stable pointer — and `recommendations.md` gains the matching signal. The rule text names the class, never the vendor; the vendor lives in the registry, where vendors belong.

## [9.27.0] — 2026-09-04

**Hook registration survives an install that moves.** `kerby install` wrote each hook's absolute path into `settings.json`. That path is only as durable as the directory it names, and the directory moved twice on the maintainer's own machine — a `~/.claude/skills/kerby` symlink that vanished, then a per-session plugin path — leaving nine registrations and the git pre-commit scanner pointing at nothing. Every one of them failed silently: Claude Code reports a missing command as a non-blocking error the model never sees, and the git hook's own guard did what it was told and exited 0. The gate was down for weeks and nothing said so until `kerby status` was run by hand. That is the class of failure kerby exists to make loud.

- **A user-local launcher, and a pointer `load` refreshes.** `install` now copies `resources/scripts/hook-launcher.sh` to `~/.claude/kerby/bin/hook` and registers every install-owned hook as `hook <event> <relpath>`. The launcher resolves the install root at hook time from `~/.claude/kerby/install-root` — one line that every `kerby load` writes from the copy it actually loaded — and runs the script as a child with stdin untouched. It reads nothing from the environment: a project settings file can set env vars for its hooks, so honoring `KERBY_DIR` would have let workspace content steer a user-local launcher. Known ceiling: one pointer per user, so two live sessions on different kerby copies share it and the last `load` wins — both are trusted installs, and the heartbeat names which one the hooks run from. Move the install, run any session once, and every hook follows. The pointer is written only from the locator's trusted rungs, never from a pin or the workspace, so the indirection changes where the path lives, not whose path it is. The git pre-commit hook goes through the same launcher in `git-hook` mode. External rulebooks keep bare paths: they do not live under the install root and do not move with it.
- **It fails open, visibly, at every tier.** A missing pointer or script exits 0 and says so on the channel the event actually reads — `additionalContext` for `PreToolUse`, plain stdout for `SessionStart`, stderr for the git hook. The launcher is POSIX `sh` with no `jq`, for the same reason the git-hook path already avoided it: GUI git clients run hooks with a bare `PATH`. It refuses an absolute or `..` relpath, because the relpath comes from a settings file the workspace can influence, and it resolves the target's directory so a symlinked directory cannot walk out of the root (a symlinked *file* inside kerby's own install is trusted content, and the header says so). A script whose interpreter is missing is a visible fail-open, not a silent exit 127. A self-test of twenty-odd cases in the same commit, run under `sh` and `dash` with a sandbox `PATH` that has no `jq`.
- **A heartbeat, first line of every session that registers the SessionStart hook.** `session-start-context.sh` now opens with `kerby engine <version> at <root> — launcher: ok; pointer ok`, or names the problem on either side: launcher missing, not kerby's, not executable, or outdated; pointer missing, dead, or naming a different copy. The root is the script's own location — the copy actually executing — so the line can diagnose a stale pointer rather than trust it.
- **The engine has a version the install can read.** `skills/kerby/VERSION` joins the five release manifests as a sixth parity surface, asserted by `check-skill-compat.py`. `load` announces `engine: kerby@<version>`; `check-updates` gains an engine row that fetches the released version (user-invoked, so the network is allowed) and touches `~/.claude/kerby/last-check-updates`; `load` prints one offline advisory when that file is absent or older than 30 days. kerby does not self-update and does not guess how a given machine updates its plugins — it names the gap and stops.
- **`install`, `status`, `hooks`, and `uninstall` all learn the new shape.** The "managed?" predicate gains a launcher-form arm and keeps the bare-path arm as *legacy*, matched under any root current or vanished, so the nine dead entries above prune as `script missing` on the next `install`; a live legacy entry is rewritten in place. `install` stages the launcher copy and the pointer write and applies them only after the single confirmation — first, so no settings entry is ever written without a verified launcher — and Phase 3 runs the same preflight, so a skipped Phase 2 cannot leave a git hook without its launcher. A registration counts only in its exact canonical shape; a launcher-prefixed entry with anything appended is reported as noncanonical and pruned. One binding predicate — launcher present, marked and executable; canonical entry; target executable under the pointer's root — is shared by `status`, `hooks`, and the Phase 3 enforcing test. `status` reports `launcher:` and `install-root pointer:` lines beside `install root:`. Windows keeps working under Git Bash; the Copilot-into-PowerShell gap is unchanged and still documented.

## [9.26.4] — 2026-09-04

**swe 2.11.4 — the provenance guard was not portable, and the platform hid it.** Three defects found by the GitHub-side Codex review, which runs on Linux. **Two of the three depend on things the local review does not exercise** — the scanner implementation and the platform's `PATH_MAX` — and that review runs on this maintainer's macOS with BSD awk, so it structurally could not see them. Note the first of those is awk-specific rather than OS-specific: the same fail-open reaches macOS too if mawk is installed. The third — a CSS colour read as a commit SHA — reproduces anywhere and was simply missed. kerby ships to other people's machines, so "passes here" was never the claim worth making.

- **A CSS colour was read as a commit SHA.** Tokenising on every non-alphanumeric discards a leading `#`, so `#1a2b3c` became `1a2b3c` and matched — and a brand colour is exactly what a real `Next Up` row carries. CSS colour *shapes* — `#` plus exactly 3, 4 or 6 hex digits — are now skipped before the SHA scan. Only those: eight is an ordinary git abbreviation length, so exempting it as RGBA hid a real SHA (the counterexample was this repo's own HEAD), and an 8-digit RGBA literal therefore reads as a SHA. Six stays ambiguous and is resolved in favour of the colour. The same token *without* the `#` still fails, asserted as a paired test so the fix cannot be "achieved" by weakening the check.
...
- **The mechanism, named plainly, because it generalizes.** Those numbers had been bumped by the same agent an hour earlier, so they felt *known* rather than *lookup-able*. § Accuracy already forbids referencing a value you have not read — it was being enforced elsewhere the same day — but a fact you authored yourself does not trip the read-before-you-assert reflex. § Status Tracking now says so directly: a fact you wrote is exactly the one to re-read.
- **Every restated field is deleted, not corrected.** Versions live in the manifests, commits and branches in git, history in the append-only `memory.log`, ticket state in the tracker — a precedence `communication.md` already set and this file was quietly violating. A copy one tier below its authority can only drift, so `Recent Completions`, `Session History`, the working branch, the timestamps and the per-status counts are gone from both the live file and the engine template. What survives is judgment that exists nowhere else: the phase, the milestone, what is queued, what is blocked. A count is the specific trap — it can disagree with the list beside it, which a list cannot.
- **The absence is guarded, and the guard ships with its test.** `scripts/check-status-provenance.sh` asserts the file states no semver, no SHA-shaped hex token, and nothing else. **It checks two things, not three: branch names are deliberately not guarded at all.** A branch check was written three times — an unanchored regex, then word-boundary matching with path-shape rejection, then keying on a "branch" label — and each design fixed its cited cases and produced new ones, because `docs/README` is both a valid branch name and a common file path and no pattern separates them. Rather than sharpen it a fourth time, it was deleted: this repo already retired a guard for under-matching, on the principle that a guard which quietly misses turns *nobody checked* into *the check passed*. The branch field is gone from the template, so that failure is now structurally prevented instead of detected, and a hand-written one falls to the rule text and review — where the wrong-`Phase`-sentence residual already sits. Deletion is what makes this checkable: an absence matches *shapes*, not phrasings, so unlike the commit-time gate rule — where three guards were each defeated by ordinary authoring wording — the ordinary ways of writing these facts are caught. It is not exhaustive, and every gap it knows about is written into the script's header: branch names entirely, a SHA abbreviation that is all letters or all digits, a four-part version, and a wrong `Phase` sentence. **It has exactly one way to exit 0** — the file was opened, read, and stated neither. A path it cannot open is reported, never passed, absence included: four fail-opens during review all came from trying to decide *why* an open failed, so it stopped asking. `check-status-provenance.test.sh` covers all of it in the same commit, including a false-positive probe, so tightening the patterns into something that fires on ordinary prose gets caught rather than deleted by the first person it annoys.
- **What it cannot do is stated rather than papered over.** A wrong `Phase` *sentence* is prose, and prose is what the doctrine says not to guard; "Shipped" when nothing shipped still gets through. Deleting the fields that made vagueness look precise reduces that residual without removing it. The independent review is what covers the rest.
- **Section order changed for a mechanical reason.** `session-start-context.sh` injects `head -30`, and `Next Up` — the only section telling the next session what to do — sat outside that window. It is now inside it. The hook itself is untouched.

## [9.26.2] — 2026-09-03

**swe 2.11.2 — one more site stops picking a deferral sink for itself.** `guardrails.md` § Where a finding goes is meant to be the only place that decides which sink a deferral lands in, with every other mention carrying a pointer and no restated detail, so there is nothing to drift. That is the intent, not yet the state of the corpus. `vendor-adapters.md` § Existing-Code Rule still named `ROADMAP.md` outright, which in a repo using an external tracker — or one with no roadmap at all — pointed at a file that does not exist. It now defers, keeping the part that was load-bearing: a vendor migration is its own scheduled workstream, never a side effect of touching a legacy file for some other reason.
...
- **The stop stays a standing condition; only its *renewal* is narrowed.** Arriving at the approval point without an approval was never this rule's business — that is the standing condition, owed whether you got there at grading time or partway through. What narrowed is what *ends* an approval you already hold: the plan being amended, or the work acquiring a property the approval never saw. A re-grade that moves the number without either is a line, not a stop.
- **Amendment is tested against whatever artifact carried the approval,** because that is not always the `plan:` line: its slots where the line stands alone, the full plan block's substance — Expected Outcomes included — where the grade called for one, and the covered scope of a plan approved in plan mode. Defining it over slots alone would have re-bound the approval to an artifact that two legitimate paths remove, which is the same shape as the 2.11.0 bug where the stop hung off the waivable plan block. The block's own complexity rating is deliberately excluded: treating a re-grade as an amendment would reinstate the fatigue this release exists to remove.
- **The second trigger is not decoration.** Slot-truth tests whether your *declaration* is accurate, not whether the *work* is still what was approved. A rate-limit constant changed inside an already-named file, an approach swapped in place, a step that became irreversible, a re-grade into the Critical row — every one of these *can* happen with all three slots still reading true. The loose "shape moves" rule covered these by accident; narrowing without naming them would have traded a fatigue bug for a coverage hole.
- **A re-grade downwards never retracts an approval**, and `references/guardrails.md` § Scope Discipline is marked as the residual it always was: a question about whether extra work belongs in the task at all, owed at every grade, and not a substitute for this gate.

## [9.26.0] — 2026-09-02

**swe 2.11.0 — a plan before every change, and the smallest change that still finishes the job.** Below grade four an agent could start editing with nothing stated: the developer found out what changed by reading the transcript afterwards. The Plan Gate now decides how big the plan is, never whether there is one. One line does it, on every change route at every grade: `plan: <files> — <change> — <check>`. Nothing substitutes for it and nothing replaces it — the grade decides what is added on top, never whether it exists. It is emitted before the first edit that implements the task, not at the grade, because § 2.5 runs before the workflow file is read and a file list written there would be a guess about files nobody has opened. Housekeeping the workflow itself prescribes first — scaffolding `agent-context.yaml`, branching, installing dependencies, repairing a baseline — is not the change and does not trip it. That is the opposite call from `rung:` on purpose: `rung:` binds at the decision point because it is a decision and survives contact with the code; a file list does not exist as knowledge until you have looked.

- **Three things waive the line:** the `investigate` route, the `prepare` / `audit` sub-commands — not because those never write, but because neither runs the § 2.5 grading this gate hangs off — and a plan the user approved in plan mode, approved rather than drafted, and only for the work that plan covers. Work outside it re-arms the line, and so does any plan whose file set turns out to be wrong.
- **The user opt-out shrinks the plan instead of removing it.** `just do it` now drops the full block to the one-line floor rather than waiving planning. That makes the floor an exception to the § 1 priority order, so § 2.5 declares it rather than leaving it to be discovered. It is not the only rule that outranks an instruction — the destructive-git refusals, the secrets rules and the base floor all do — but those are safety floors and this one is not, so it is the one that has to argue for itself.
- **Minimal is not partial.** `feature.md` § 3 gains `smallest:` and `deferring:`. What may be deferred is hardening beyond the ask, refactors, adjacent cleanup and polish — never a piece of the requested behavior, and never anything on § 1b's never-defer list, which now names the requested behavior and tests for behavior you changed alongside the four it already carried. On the bugfix route the minimal version is the root-cause fix; a symptom patch is never the minimal version, whatever its line count.
- **A deferral has to survive the session that made it.** `ROADMAP.md` gains `## Follow-ups`, exempt from the active list's complexity rule of thumb and named as a category rather than a horizon so the no-Now/Next/Later prohibition still holds. Every item named in `deferring:` appears word for word in the report's `skipped:` line and in the deferral sink, which `references/guardrails.md` selects. Three places a reviewer can compare, which is what separates this from a good intention.
- **Three rules said the opposite and are fixed in the same change, not after it.** Anti-Rationalization's "follow-up PRs are the slowest PRs" now names the *unrecorded* follow-up as the excuse; Code Standards' "the trigger lives at the code site" is scoped to shortcuts that have a code site; and Scope Discipline gains a four-sink table so a deferral, a human action, an in-code shortcut and an adjacent finding stop competing for one log. An issue you merely noticed in adjacent code is still a neutral observation — promoting one to a follow-up to make it look actioned is the failure this split exists to prevent.
- **`quick-task` verifies its risk criteria before the first edit.** Four of them — test, schema/contract, high-stakes path, new logic — are knowable from the file list, so they are checked before anything is written rather than after the diff. Only the LOC budget genuinely needs the diff. Either way an escalation re-grades and re-emits the plan before going on; what differs is the code already written — an LOC overrun leaves it standing, a risk criterion means saying what reached the tree without its gate and offering to revert. The fit check stays a risk declaration, not a substitute for the plan line.

No hook change, no new gate. The rule is behavioral, like the rest of the Plan Gate.
...
- **A decline can now delete.** This is the half that made the rest work. `install` used to prune only entries whose script had vanished, on the reasoning that anything still resolving was a live binding worth keeping. Sound while install could only add — but a live binding is exactly what a decline has to remove, and left alone, declining a hook would have been silently inert for anyone who had already installed: the old entry kept firing while `status` reported everything healthy. The prune now also removes entries whose `(event, matcher, path)` tuple is absent from the derived set, at handler granularity so a sibling sharing a matcher group survives untouched. Hooks outside every kerby root are still never touched.
- **Removals that drop a protection get their own callout**, above the diff rather than inside it. Declines are not recorded anywhere, so a re-run re-asks every declinable row, and one mis-keyed `n` could delete a binding accepted months ago. Default-yes prompts, a visible deletion line, and a final confirmation already stood between that and disaster; the callout closes the last gap — a single unwanted deletion hiding among several wanted ones.
- **`status` reports orphans.** A managed entry whose script exists but whose tuple is no longer derived now reads `orphaned registration — re-run kerby install`. The orphan is an extra handler nothing points at, and without this row the prune that clears it had no way to be discovered. Whether the check itself also reads degraded depends on the case — bound if the correct tuple is registered alongside the orphan, unbound if only the stale one survives — and the two are reported independently rather than inferred from each other.
- **`locked` never silences an external rulebook's prompt.** A `local` or `remote` rulebook's enforcers are confirmed one at a time regardless of what its manifest declares, or any rulebook could force a script into your settings by setting one boolean. Trust comes from where a rulebook resolves, not from what it says about itself.

## [9.23.0] — 2026-08-22

**Hooks get tiers, and a guard that proves them**: kerby has always known which hooks may be switched off at runtime and which may not, but it knew it as a hand-written table — one that fell behind every time the hook set moved, and which omitted `codex-pr-gate` entirely. The knowledge was already in the manifests: `floor = true` means nothing may loosen this, and `severity` says whether a finding denies or merely holds. Those two fields name three tiers — `locked`, `recommended`, `optional` — and the table collapses into a rule: **a hook honors `CODING_RULES_HOOK_DISABLED` if and only if its tier is `optional`.** All twelve shipped scripts agree with it today. `scripts/check-hook-disable-tier.sh` checks the mechanical half of it — that each script declares, or does not declare, the canonical disable block its tier calls for — enumerating enforcers from the manifests rather than globbing the hooks directory, and reporting anything it cannot classify as an error rather than a pass. It deliberately stops short of verifying runtime behavior: three review rounds established that inferring that from script text cannot be made sound, so the claim was narrowed to fit the evidence instead of the evidence stretched to fit the claim. The header says what that leaves uncaught. Both shortcuts were tried during design and both returned confidently wrong answers — one silently skipped `hollow-test-check.sh` because the filename contains "test", the other mistook three comments saying *we refuse this* for the opposite. No manifest field was added; a `tier` key would have been unvalidated, since `[[check]]` has no unknown-key rejection.

- **`floor` is enforced everywhere, and the contract now says so.** The field's row claimed it was "only meaningful in `base`". The validator disagreed — it drives E05 and E06 across the whole merged set, so swe's `floor = true` on `destructive-git` has been live all along. The row also gains the tier's one safety scope: `locked` binds install-resolved builtins only, because an external rulebook that could mark its own hook non-declinable would be force-registering it by setting a boolean.
- **`override` is documentation, and now reads like it.** Nothing in the engine consumes the field — it appears exactly once, as its own contract row. The real escape hatches live in the enforcer scripts (`CODING_RULES_ALLOW_PROTECTED_COMMIT=1`, `CODEX_GATE_BYPASS=1`), and nothing verifies that a check declaring `override` implements one. The row now says so, distinguishes it from the unrelated `override_of` that *is* read, and requires that any future behavior reading it gain validation in the same change.
- **`status` stops guessing at intent.** An unregistered enforcer read `degraded — run install to bind`. Declines are never recorded, so that label was both a guess and a nag: it could not tell a deliberate choice from a machine where install had simply never run, and it told the former to undo itself. It now reads `not registered (behavioral only)`, with a standing note for `recommended` checks.
- **`status` names its install root.** A skill install is a separate copy from the repo it was built in, and it may sit behind a symlink. Every other line looks identical whether or not the two agree, so a change made in one and a session running the other are indistinguishable. One line settles it.
