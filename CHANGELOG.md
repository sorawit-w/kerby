...
**swe 2.11.0 — a plan before every change, and the smallest change that still finishes the job.** Below grade four an agent could start editing with nothing stated: the developer found out what changed by reading the transcript afterwards. The Plan Gate now decides how big the plan is, never whether there is one. The floor is a single line — `plan: <files> — <change> — <check>` — emitted immediately before the first edit, not at the grade, because § 2.5 runs before the workflow file is read and a file list written there would be exactly the invented path § Accuracy forbids. That is the opposite call from `rung:` on purpose: `rung:` binds at the decision point because it is a decision and survives contact with the code; a file list does not exist as knowledge until you have looked.

- **Three things waive the line, and each is a case where a plan already exists or nothing is changing:** the `investigate` route, the `prepare` / `audit` sub-commands, and a plan the user approved in plan mode — approved, not drafted, and only for the work that plan covers. Work outside it re-arms the line, so one approval does not cover a session.
- **The user opt-out shrinks the plan instead of removing it.** `just do it` now drops the full block to the one-line floor rather than waiving planning. That makes the floor the single place this playbook outranks an explicit user instruction, so § 2.5 says so outright rather than leaving it to be discovered — an undeclared exception to the priority order is the problem; a declared one is a decision.
