...
- **The stop stays a standing condition; only its *renewal* is narrowed.** An approval already given ends on three things, each firing once — the grade reaching the approval point from below, the plan being amended, or the work acquiring a property the approval never saw. A re-grade that moves the number without any of those is a line, not a stop.
- **Amendment is defined twice on purpose,** because the `plan:` line does not always exist: a slot that stopped being true where there is a line, and work moving outside what an approved plan-mode plan covered where there is not. Defining it only over slots would have re-bound the approval to an artifact a legitimate path removes — the same shape as the 2.11.0 bug where the stop hung off the waivable plan block.
