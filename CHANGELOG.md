...
- **The absence is guarded, and the guard ships with its test.** `scripts/check-status-provenance.sh` asserts the file states no semver, no SHA-shaped hex token, and, on any line that says "branch", no `<type>/<name>` reference for the seven documented types and no bare protected-branch name. **The branch check keys on that label rather than on token shape** — `docs/README` is both a valid branch name and a common file path, so two rounds of shape heuristics traded false positives for missed branches without converging. Keying on the label matches how the field actually appears and removes the ambiguity instead of patching it. Deletion is what makes this checkable: an absence matches *shapes*, not phrasings, so unlike the commit-time gate rule — where three guards were each defeated by ordinary authoring wording — the ordinary ways of writing these facts are caught. It is not exhaustive, and every gap it knows about is written into the script's header: a branch named on a line that never says "branch", a SHA abbreviation that is all letters or all digits, a four-part version, and a wrong `Phase` sentence. `check-status-provenance.test.sh` covers all of it in the same commit, including a false-positive probe, so tightening the patterns into something that fires on ordinary prose gets caught rather than deleted by the first person it annoys.
