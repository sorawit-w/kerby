...
| Builtin version-reconcile (v9.10.0) | a pinned load/reload whose builtin pin `version` differs from the installed manifest adds one line — `pin updated: <id> <old> → <new> (builtin — tracks the install)` — then re-pins that entry from the install; fires once per version change, quiet after. Externals unaffected |
