...
| Intent manifest (v9.11.0) | new selection rung between pin and detection: a no-pin load with `.kerby/rulebooks.toml` present resolves its entries (builtins by id; `source` URLs through the full TOFU'd remote flow; local externals named + skipped), then writes the ordinary lock. While the manifest exists, pin mutations and version-reconciles mirror into it with an `intent updated: …` line; a malformed manifest announces `intent manifest unreadable (<reason>) — ignoring; falling through`. `install` gains a create-offer after lock hygiene. Announcement grammar gains `intent` as a selection source; a pinned load adds a read-only `intent manifest also names: <ids> — load <id> to adopt` advisory when the manifest lists unselected ids. Spec-tested, not parity-tested |
