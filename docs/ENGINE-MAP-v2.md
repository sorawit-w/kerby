...
| Builtin version-reconcile (v9.10.0) | a pinned load/reload whose builtin pin `version` differs from the installed manifest adds one line — `pin updated: <id> <old> → <new> (builtin — tracks the install)` — then re-pins that entry from the install; fires once per version change, quiet after. The reconcile resolves before the announcement renders, so the announced `<id>@<version>` is always the installed (loaded) version — the stale number never appears outside the `pin updated` line. Externals unaffected |
| `install` lock hygiene (v9.10.0) | + one confirmed step riding the pin write: `git check-ignore` test, then an `Add to .gitignore? [y/n]` offer showing the exact two-line marker block; `uninstall` gains the symmetric exact-marker-block removal offer. `load` may append a one-line gitignore tip to a first-pin announcement (tip only, no write) |
| `status` panel (v9.10.0) | + stale-builtin header form `<id>@<pin-version> (builtin — install has <new-version>; next load re-pins)`; report-only, no lockfile write |
| Intent manifest (v9.11.0) | new selection rung between pin and detection: a no-pin load with `.kerby/rulebooks.toml` present resolves its entries (builtins by id at the manifest's declared version, reconciled by step 2; `source` URLs behind a **per-entry pre-fetch confirmation**, then the full TOFU'd remote flow — headless skips them, named, and writes no pin; local externals named + skipped; zero resolved entries → no pin, fall through), then writes the ordinary lock. While the manifest exists, pin mutations and version-reconciles mirror into it with an `intent updated: …` line; a malformed manifest announces `intent manifest unreadable (<reason>) — ignoring; falling through`. `install` gains a create-offer after lock hygiene. Announcement grammar gains `intent` as a selection source; a pinned load adds a read-only `intent manifest also names: <ids> — load <id> to adopt` advisory when the manifest lists unselected ids. Spec-tested, not parity-tested |
| `check-updates` (v9.11.0) | new engine command (reserved; E13 covers it) — read-only freshness report: builtins compared offline vs the install, remotes shallow-fetched to a session temp dir and compared by version+hash, locals named as uncheckable; per-entry lines + one-line footer. No baseline exists — spec-tested, not parity-tested |

| `status` unregistered-enforcer label (v9.23.0) | `degraded — run install to bind` becomes `not registered (behavioral only)`, with `— recommended; not enforcing until kerby install binds it` appended for a `recommended`-tier check. Declines are never persisted, so the label states *state*, not intent — nothing can distinguish "declined" from "install never ran here", and the old wording additionally nagged a user to undo a deliberate choice. The section also now forbids drawing an operational conclusion from an unregistered protection (an evaluator run closed such a panel with "your overnight `.env` writes are unblocked") |
| `status` install root (v9.23.0) | + one line, `install root: <path>`, printed above the `Loaded rulebooks:` header. Answers which copy of kerby the session is reading — a skill install is a separate copy from any repo it was built in and may sit behind a symlink, so no other line reveals that mismatch |

| `install` Phase 2 prompt (v9.24.0) | the single `[y/n]` becomes **table-then-question**: a derived table (tier / event / matcher / enforcer / checks-it-binds as `<id> (<kind>, <enforcement>)`, `gap` appended verbatim for `partial`; no prose description column, as `[[check]]` has no field for one) printed *before* an `[all / choose / none]` prompt; `choose` walks the `recommended` and `optional` rows one at a time, each defaulting to yes. `locked` rows are shown and never offered *individually* — `none` still writes nothing at all, tiers govern which rows may be declined one at a time, not whether Phase 2 registers. External (`local`/`remote`) enforcers are confirmed one at a time on the `all`/`choose` paths — `all` is not consent to them, and their tier is irrelevant since the confirmation is a trust step. `none` ends Phase 2 before any external confirmation is asked. Supersedes any baseline flow in which one answer covered every enforcer. The v7-baseline `scenario-5-install-hooks-table.md` equality class is (event, matcher, filename) tuple-set, which an `all` answer reproduces exactly **for a builtin-only selection** — a selection including an external rulebook depends on its per-hook answers |
| `install` Phase 2 prune (v9.24.0) | the remove set gains entries whose `(event, matcher, resolved path)` tuple is **absent from the newly derived set** — declined this run, or a changed `matcher`/`event` — alongside the existing dead-script entries. Matched at **handler** granularity inside a matcher group, so a sibling sharing the group survives. The prior rule forbade pruning any entry whose script still resolves; that is replaced, because a decline must be able to remove a live binding or it is inert wherever the hook was already registered. Tuple-absence pruning is scoped to **project** settings files; in global `~/.claude/settings.json` only dead-script entries are pruned, because another project's registration is indistinguishable from a decline. The comparison universe is the fixed engine services **plus** the manifest-derived enforcers — a manifest-only reading would orphan the SessionStart trio. A removal that drops a `locked`/`recommended` protection, **or one whose tier cannot be recovered**, is called out in its own block above the diff. Phase 2 summary reports removals by cause; the causes are not disjoint (a declined hook is also underived), so a removal is reported under the most specific cause that applies |
| `status` orphan row (v9.24.0) | + `orphaned registration — re-run kerby install` for a managed entry whose script exists but whose tuple is not in the derived set. The orphan is an extra handler nothing points at, and without this row the prune has no discovery path. Whether the *check* also reads degraded depends on the case: if the correct tuple is registered alongside the orphan the check stays bound, but after an `event`/`matcher` change where only the old entry exists, the current tuple is unbound and the check reads `not registered (behavioral only)` **as well as** producing an orphan row |

| `hooks` (v9.25.0) | new engine command (reserved; E13 covers it) — read-only listing of the Phase 2 **candidate set** with each row's binding state against one named settings file, plus the orphan / missing-script rows `status` reports. Renders exactly what Phase 2's table renders (no prose description column) and writes nothing. The external render-trust rule is inherited verbatim from `commands`; cold behavior is stated explicitly instead, since `hooks` must resolve a merged selection (pin → intent manifest → detection, never the ask, never a write) where `commands` needs none. `STATE` requires tuple equality **and** an existing script; an unparseable settings file yields `unknown` for every row rather than a fabricated `not registered`; an entry under an unapproved external is `unclassified`. No baseline exists — spec-tested, not parity-tested |
