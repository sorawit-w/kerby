...
| Intent manifest (v9.11.0) | new selection rung between pin and detection: a no-pin load with `.kerby/rulebooks.toml` present resolves its entries (builtins by id; `source` URLs through the full TOFU'd remote flow; local externals named + skipped), then writes the ordinary lock. While the manifest exists, pin mutations and version-reconciles mirror into it with an `intent updated: …` line; a malformed manifest announces `intent manifest unreadable (<reason>) — ignoring; falling through`. `install` gains a create-offer after lock hygiene. Spec-tested, not parity-tested |
