...
# Exit 0 = allow action (always); stderr surfaced to the agent as feedback.
#
...
#
# GLOB SOURCE — canonical list is BOOTSTRAP §3 (resources/BOOTSTRAP.md, the
...
for g in "${GLOBS[@]}"; do
  ...
  if printf '%s' "$target" | grep -qiE "^${re}$"; then
    echo "NOTE (kerby): $FILE_PATH is a high-stakes path (BOOTSTRAP §3 — auth/migrations/payments/infra/CI). This change requires workflows/feature.md or bugfix.md + the §4 Plan Gate, NOT quick-task — even for a one-liner. (Production-traffic-shaping paths aren't glob-matchable and stay your judgment call.)" >&2
    break
  fi
done
