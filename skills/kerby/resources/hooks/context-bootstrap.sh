...
#   from templates/CONTEXT.md.template (resolved via $KERBY_DIR
...
# Resolve template path. Prefer $KERBY_DIR; fall back to script-relative.
SCRIPT_DIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" >/dev/null 2>&1 && pwd )"
TEMPLATE=""
if [[ -n "${KERBY_DIR:-}" && -f "$KERBY_DIR/resources/templates/CONTEXT.md.template" ]]; then
  TEMPLATE="$KERBY_DIR/resources/templates/CONTEXT.md.template"
