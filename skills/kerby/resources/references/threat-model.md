...
| Destructive git (`push --force`, protected-branch push, `reset --hard`, `clean -f`, `branch -D`, wholesale discard) + commit while on a protected branch | `protect-git.sh` (PreToolUse Bash) hard-block | `[enforced-when-installed]` | Regex-matched on the command string; exotic shell obfuscation could evade — `protect-git.test.sh` covers the common forms. The commit gate parses the git subcommand (so `git log --grep=commit` is not a commit), reads the **target** repo's live branch (resolving a single `git -C <path>`), and is *escapable* via an inline `CODING_RULES_ALLOW_PROTECTED_COMMIT=1` prefix (workflow guard, not data loss) — the destructive blocks are not. **Residual:** a single PreToolUse pass can't fully model runtime git — multiple `-C`/`--git-dir`, an unknown global option before `commit`, or heavy quoting can still evade or mis-resolve. A git `pre-commit` hook (runs in-repo at commit time) would be bulletproof but is a different mechanism; not adopted here to keep one install model |
