...
5. If approved, agent writes the entry file and runs `bash "${KERBY_DIR}/resources/hooks/knowledge-reindex.sh" --force` to refresh the AUTO-INDEX block in `KNOWLEDGE.md`. (If the hook isn't available — e.g., on a platform without `kerby` wired — the agent updates `KNOWLEDGE.md` by hand, adding one line in the format `- [Title](filename.md) — one-line summary` between the AUTO-INDEX markers.)
