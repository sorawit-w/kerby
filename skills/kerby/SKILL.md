...
3. **Detection (builtins only, v9.1)** — for each installed **builtin** whose manifest declares `[detect]`, match its `markers` (root-relative globs) against the project root with `Glob`. **Exactly one builtin matches → select it**, `source: detected`. **Two or more match → ask** the user which to load (multi-select; co-loading is ordinary — `selected` takes several), `source: chosen`. A skill repo that also ships code is the *expected* multi-match (`swe` + `skill-authoring`) — ask, never guess. External rulebooks' `[detect]` is never consulted (D19): workspace shape may steer *among install-trusted builtins*, never toward external content.
...
7. **Readiness nudge (read-only).** After confirming, check whether this repo is already prepared for kerby, and suggest `prepare` if not. This adds **no writes** — detection only.
   ...
   - **Has real code?** True if any project manifest exists (`package.json`, `deno.json`, `pyproject.toml`, `go.mod`, `Cargo.toml`) or there is a populated source tree. (This list is intentionally the same as `swe`'s `[detect].markers` — keep the two in sync; the nudge and detection answer the same "is this a code repo?" question.)
