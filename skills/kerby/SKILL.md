...
2. If `selected` is empty (a valid floor-only pin — § `load`), report via the existing floor verdict: `floor present — no governing rulebook selected` (and suggest `load <id>`), never a vacuous "loaded" over zero rulebooks. Otherwise, if every selected rulebook's markers are found, report — naming only the rulebook ids, never the matched phrase text:
