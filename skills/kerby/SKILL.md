...
3. **Intent manifest (`.kerby/rulebooks.toml`, optional, committed)** — the lockfile's committed inverse: path-free, trust-inert; schema + doctrine in `docs/rulebook-contract.md` § Intent manifest. Reached only when there is no pin (the fresh-clone case). Resolve each `[[rulebook]]` entry: a **builtin id** → against the install (a `version` mismatch is announced via load step 2's reconcile line — the manifest's exact pin is a drift alarm, not a resolver); an entry with **`source`** → the full remote flow, **TOFU prompt included** (the manifest never pre-approves; a declined prompt skips that entry, named, and the load continues); an entry with **neither** (a local external — no shareable source) → named and skipped: `local rulebook — supply your own copy`. The resolved selection then writes the ordinary lock; each resolved entry announces with `source: intent`. **Malformed** (TOML parse failure, unknown field, non-slug id, non-URL `source`) → announce `intent manifest unreadable (<reason>) — ignoring; falling through` and continue to detection — the manifest grants no trust, so skipping it is safe; silence is not.
...
Freshness report for the current selection. **Read-only — this command writes nothing** (no lockfile, no intent manifest, no materialization); it touches the network only because the user invoked it, consistent with the no-silent-network rule (§ Remote sources). Read the pin, and the intent manifest if present; when neither exists, say so and stop.
