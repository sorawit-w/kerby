...
2. Resolve each selected rulebook's `rulebook.toml` and validate per the trust section (hash-keyed; trust prompt for first-load/changed local rulebooks; fail-closed → HELD). Merge `base` first. **Builtin version-reconcile:** for each `selected` entry with **install-resolved builtin identity** (never the id string alone — § Rulebooks, selection, and trust), if the pin's `version` differs from the installed manifest's, the pin is stale — builtins track the install, so the field is lying. Rewrite that entry **entirely from the install** (every field re-derived, the pin's `path_or_url` never read — the same doctrine as § Migration residue) and announce one line: `pin updated: <id> <old> → <new> (builtin — tracks the install)`. **The reconcile resolves before the per-rulebook announcement renders**, so the announcement line always carries the installed version (the one actually loaded); the stale number appears only inside the `pin updated` line, never as the announced `<id>@<version>`. After the re-pin the versions agree, so the announcement fires once per version change, then goes quiet — no extra state. **Externals are exempt:** their version only changes with their content, which already routes through the hash → reapproval gate; rewriting an external's pin fields outside that prompt would be a silent trust mutation.
...
4. **Rulebook panel.** After the loaded/not-loaded verdict, print a `Loaded rulebooks:` header line listing each selected rulebook as `<id>@<version> (<origin>)` (plus `base (floor — always loaded)`), then report the rulebook state so degrade is visible, never assumed:
   ...
   - **Git-hook binding (only for checks declaring `git_hook`).** Degrade must be visible, but **report only states `install` or `uninstall` can act on** — a label nobody can act on is the loop this section already shipped once. Check `core.hooksPath` with `git config --get --show-origin core.hooksPath` **first and directly**; do not resolve it with `git rev-parse --git-path hooks`, which returns the *configured* dir and therefore can never find a dormant file in `.git/hooks` to report on.
     **Which file:** always the repo's **default** hooks dir (`<git-common-dir>/hooks`), never the dir `core.hooksPath` points at. Compare the two as resolved paths before calling anything shadowed: `core.hooksPath` set *to the default dir itself* changes nothing, and reporting that as dormant would be wrong in both directions — git is running kerby's hook. kerby writes there and only there — Phase 3 refuses when `core.hooksPath` is set — so that is the only place its hook can be, and looking through the config instead would both miss a dormant hook left from before the config changed and pick up files kerby never wrote.
     - `git-hook: installed but not running — core.hooksPath=<value> (<origin>)` — git runs hooks from that dir instead, so kerby's file is dormant. It is not gone, and it will start running again if the config is removed. `kerby uninstall` removes it; `install` will not add one while the config is set.
     - `git-hook: not enforcing — core.hooksPath=<value> (<origin>)` — the config is set and kerby has no hook here. Note this says nothing about whether *some* hook runs: a manually wired one in the configured dir may well be enforcing. kerby only reports on its own.
     - `git-hook: installed` — present, byte-identical, executable, and its scanner passes `-x`.
     - `git-hook: installed but its scanner is not executable` — the hook is ours and runs, but the enforcer it points at is missing or not `-x`, so the hook's own guard exits 0 on every commit. Repair the kerby install itself; re-running `install` cannot conjure back a scanner that is not there. Test `-x`, never mere existence — a `0644` scanner passes an existence check and still fails the guard.
     ...
     - `git-hook: a different pre-commit hook is present` — not byte-identical to ours, whoever wrote it and whenever. kerby will not modify or remove it; if it *was* kerby's and the install has since moved, remove it yourself and re-run install.
...
3. **A hook already exists there** → read it. Three outcomes, and deliberately no more:
   - **Byte-identical** to what we would write → it is ours and current, but a matching body proves neither of the two things that decide whether it actually enforces. Check both:
     - **executable?** git skips a non-executable hook silently — `chmod +x` if not. This is the only in-place change install ever makes to an existing hook.
     - **is its enforcer EXECUTABLE?** Test exactly what the hook's own guard tests — `[ -x <path> ]`, not mere existence. A scanner present at mode `0644` satisfies "exists", fails the guard, and the hook exits 0 on every commit while both this branch and `status` report green. The body matching means the path is the *current* one, so a failure here means kerby's own install is incomplete or wrongly-permissioned; rewriting the hook cannot fix it. Say so and name the path — mirroring the guard is the point, since any check weaker than the guard produces a false green by construction.

     Both fine → `already installed`. (An earlier draft checked only the executable bit. A byte-identical, executable hook with a deleted scanner then reported `installed` while enforcing nothing — a false green on a security control, which is worse than the loop that simplification was fixing.)
   - **Anything else** → **do not touch it.** Report the path and its first line, and continue the rest of install. This covers a hand-edited hook, a hook from another tool, *and* one kerby wrote against an install root that has since moved — all three are "not the file we would write", and install does not try to tell them apart.
   - **Absent** → offer, then write.

   **Why there is no re-point branch.** An earlier draft split "ours but drifted" into *stale* (only the path differs — re-point it) and *hand-edited* (leave it). Three review rounds could not make that split hold: any test loose enough to recognise a moved install also accepted a marker plus a rewritten path, and tightening it to a fixed `rulebooks/<id>/hooks/<file>` suffix then stranded local rulebooks that have no such shape. A moved install now reads as "not ours" and the remedy is one line the user runs — `rm <path>`, re-run install — which terminates without kerby ever rewriting a file it did not just author. Fewer states that always hold beats more states that need a footnote each.
...
For each in-scope check declaring `git_hook`, look at the hook at the resolved hooks dir. Look in the same place `install` writes — the repo's **default** hooks dir, never the one `core.hooksPath` points at — and apply the same ownership test `install` uses, so the two never disagree about whose file it is. This is also why a hook that went dormant when someone later enabled husky is still found and removable:

- **Byte-identical** to what `install` writes → ours; show it as a removal and include it in the confirmation.
- **Anything else** → leave it and report `<path> is not the file kerby installs — left in place`. That deliberately includes a hook kerby wrote before the install moved: kerby cannot distinguish it from a hand edit, so it does not guess.
