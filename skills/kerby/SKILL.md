...
2. **Delete the clone's `.git/`** — avoids a nested repo/gitlink if the clone is committed, and removes content that could mutate without re-approval. The trust hash is unaffected: it is the same framed manifest+declared-files digest as always (V10); undeclared clone files sit outside it.
3. **Validate** (authoritative), then require the manifest `id` to match the slug rule — the id is untrusted and becomes a path component.
...
1. **Resolve the install root** (same locator as `load`: Glob `**/skills/kerby/SKILL.md` → parent dir, else `${KERBY_DIR}`, else ask).
   ...
   **The registration set is derived, not hardcoded (V4):**
   ...
   - **Dedup key = (event, matcher, *resolved script path* — following a one-line `exec` shim to its target):** two checks that resolve to the **same actual script** produce one entry (base's `secrets-staged` and code's `hollow-test-heuristic` both resolve to `rulebooks/base/hooks/pre-commit-check.sh` — code declares its own path via the confinement shim, which resolves through to base's script; one registration runs the scan once). **Do not dedup on filename alone** — with multiple external rulebooks, two unrelated rulebooks may each declare a `hooks/check.sh` at *different* paths; those are different scripts and both must register, or the second rulebook's selected check silently never runs. The registered path is the resolved target (base-first order for the shared-script case). An enforcer whose check declares no `event` cannot be auto-registered (the validator warns E09) — skip it and say so.
...
5. **Detect already-managed entries.** A hook entry is *kerby-managed* iff its `command` ends in one of the derived filenames AND its path is under a **kerby hook root** — `/skills/kerby/rulebooks/` (builtin enforcers + floor), the legacy `/skills/kerby/resources/hooks/` (engine trio + v7 migration shims), **or the folder of any loaded external (`local`/`remote`) rulebook** as recorded in the lockfile / user-local approval store (a remote's is `.kerby/rulebooks/<id>/hooks/`). This is the **single managed-entry signature** shared by `install`, `status`, and `uninstall` — an external rulebook's registered enforcer must be recognizable as already-present (else a re-run duplicates it), bindable (else `status` wrongly shows it degraded), and removable (else `uninstall` orphans it). Skip already-present entries — Phase 2 is idempotent. A legacy entry and its new-path equivalent are the **same** entry (resolved-path dedup). **Shim re-point nudge:** when a kerby-managed *enforcer* entry points at the legacy `resources/hooks/` path, offer once: `<N> registered hooks point at pre-v7 shim paths; re-point them to the new locations in this diff? [y/n]` — on `y`, the diff rewrites those entries to the derived paths; on `n`, they keep working through the shims until v8 removes them.
