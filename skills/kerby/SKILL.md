...
4. **Rulebook panel.** After the loaded/not-loaded verdict, print a `Loaded rulebooks:` header line listing each selected rulebook as `<id>@<version> (<origin>)` (plus `base (floor — always loaded)`), then report the rulebook state so degrade is visible, never assumed:
   ...
   - **Git-hook binding (only for checks declaring `git_hook`).** Same doctrine as above — degrade must be *visible*, never assumed. One extra clause on that check's row, resolved against `git rev-parse --git-path hooks`:
     ...
     Every state below `not installed` looks like "protected" to a user who only checks that the file exists, which is why each gets its own words rather than a boolean.
...
3. **A hook already exists there** → read it, and sort it into exactly one of four states. **Drift is not one state but two, and they need opposite handling** — conflating them is what made `status`'s remediation loop forever (below).
   - **Ours, byte-identical** to what we would write → still verify the two things a matching body cannot tell you: that the file is **executable**, and that its **exec target exists**. Both are repairable in place (`chmod +x`; re-point) and both are states `status` reports with `re-run kerby install`, so install must actually fix them — a branch that returns `already installed` while the hook is unexecutable or dangling is the same non-terminating remediation this whole section exists to prevent. All three fine → `already installed`, no diff, no prompt. Phase 3 is idempotent, like the others.
   - **Ours but STALE** — it carries the marker and is **byte-identical to the freshly rendered template except that one path string differs, consistently, at every occurrence** (it appears three times: the guard test, the guard's message, and the `exec`). Any other difference — a changed guard, an extra line, an altered message, the path differing at only some occurrences — is NOT stale.

     **The path must differ only in its install-root PREFIX.** Everything from the rulebook segment onward — `rulebooks/<id>/hooks/<filename>` — must match the current template exactly. "Somewhere under a kerby root" is too weak a test: `.kerby/rulebooks/*/hooks/` is itself a recognized kerby root, so a marker plus a rewritten tail (`…/.kerby/rulebooks/evil/hooks/pwn.sh`) would satisfy it and be silently adopted as kerby's own. Prefix-only is what makes "a moved install" the *sole* thing this branch can mean. This is kerby's own file gone out of date, not someone else's work: **re-point it in the same pass**, exactly as Phase 2 prunes and re-points stale settings entries, and show it as a diff. Without this, `status`'s `installed but its target is missing — re-run kerby install` is advice that can never come true, and the remediation loops instead of terminating (§ Phase 2, *"so re-run `kerby install` self-heals"*).
   ...
   Never chain, never replace, never back up and overwrite. The re-point case is the *only* write to an existing file, and it narrows the guarantee rather than keeping it absolute: kerby wrote every other byte of that file, but if you hand-edited *just the path* to a different kerby install and left everything else untouched, install will re-point it and uninstall will remove it. That is the accepted edge of making the remediation terminate; anything beyond a prefix-only path change is treated as hand-edited and left alone.
