...
3. **A hook already exists there** → read it. Three outcomes, and deliberately no more:
   - **Byte-identical** to what we would write → it is ours and current. `chmod +x` if the file itself is not executable (git skips a non-executable hook silently); that is the only in-place change install ever makes to an existing hook. Then `already installed` — subject to the enforcing test below, like every other outcome.
