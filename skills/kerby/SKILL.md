...
2. If the selected rulebook's markers are found, report — naming only the rulebook id, never the matched phrase text:
...
- **Pin migration (load step 1 / trust section).** A pin entry claiming
  ...
  collision first, exactly like `load +` refusing a duplicate id. Migrate
