...
**`STATE` is per settings file, and the file must be named.** Binding is a property of one settings file, not of the project — the same enforcer can be registered globally and absent locally. Examine the project's `.claude/settings.local.json` by default, name it in the header, and say so when it does not exist (`not registered` for every row, which is a true statement about that file rather than about the machine). `kerby hooks --settings <path>` examines another. Determine each row's state with the **exact-tuple test** (`install` § Detect already-managed entries): `registered` iff a settings entry matches that enforcer's tuple — `(event, matcher, relpath)` in launcher form, `(event, matcher, resolved path)` for an external — **and the script it names exists on disk** (`<pointer-root>/<relpath>` for launcher form). A live legacy bare-path entry reads `registered (legacy path)`. When `~/.claude/kerby/bin/hook` is absent, print one header line — `launcher: missing — no launcher-form row is enforcing` — rather than repeating it per row. Tuple equality alone is not binding — `status` already refuses to count an entry whose script is gone, and a table that disagreed with `status` about the same machine would be worse than no table. A matching tuple whose script is missing reads `registered script missing` in the `STATE` column, not `registered`.
...
4. **Build the hook entries** from the **accepted set** — never the candidate set. A declined row is a candidate and must not be written; building from candidates would re-add exactly what the user just declined, and the prune would not catch it because there was no prior entry to remove. Install-owned entries in launcher form, externals as absolute paths (step 1 § Written form). As a **worked example**, a bundled-`swe` selection (detected or chosen) derives exactly this set — a different selection derives a different set from its own manifests, with zero engine change:
   ...
   Each entry uses the standard Claude Code hook shape — the `command` is the launcher, the event, and the relpath, in that order (double-quote the launcher path when `<home>` contains whitespace; the command runs through a shell):
...
4a. **Launcher and pointer preflight.** A launcher-form entry is inert until the launcher exists, so before any settings write: (i) `mkdir -p ~/.claude/kerby/bin`; if `bin/hook` is **absent**, copy `<install-root>/resources/scripts/hook-launcher.sh` there, `chmod +x`, and verify `-x`; if it is **present with the `kerby-managed:launcher` marker and the bytes differ**, it is ours but outdated — show the diff and rewrite it wholesale (ownership is the marker, currency is the bytes — Phase 3 step 3); if it is present with **no marker**, it is not ours: stop Phase 2 naming the file, and write nothing. (ii) If `~/.claude/kerby/install-root` is missing, or names a copy other than the root step 1 just resolved, write it. Both writes are listed in the step 6 callout, above the diff, and are covered by the single step 7 confirmation — they are user-local, kerby-owned state (§ Rulebooks, selection, and trust), not user files. **No settings write ever precedes a verified `-x` launcher**: a registration whose launcher is missing would be a silent no-op on every tool call, which is the exact shape of failure this indirection exists to end.
...
5. **Detect already-managed entries.** Two distinct tests — do not conflate them:
   - **"Is this a kerby-managed entry at all?"** (used by `uninstall`'s sweep): its `command` is in launcher form, or its path is under a **kerby hook root**. The shapes are, in order of how they're recognized:
     - **Launcher form** — the `command` begins with `<home>/.claude/kerby/bin/hook ` (optionally double-quoted). Every install-owned registration since 9.27.0 has this shape, so the prefix alone is the signature; the relpath that follows says which script.
...
1. **The hook file is executable** (`[ -x <hook> ]`). Git skips a non-executable hook
   silently.
2. **The launcher it points at is executable** (`[ -x ~/.claude/kerby/bin/hook ]`) — *exactly*
   what the hook's own guard tests — **and the enforcer the launcher will resolve is
   executable** (`[ -x <install-root>/<relpath> ]`) — exactly what the launcher's guard
   tests. Mirror both guards, never mere existence. A scanner at mode `0644` exists, fails
...
5. Write back. Summarize Phase 2:
   ...
   On a **bare** `uninstall` only, after the settings write, ask once: `Also delete ~/.claude/kerby/bin/hook and ~/.claude/kerby/install-root? [y/n]` — say in the prompt that other projects' settings files may still reference the launcher, and leave both in place on `n`. A scoped `uninstall <rulebook>` never offers this.
