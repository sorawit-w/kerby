...
`uninstall` mirrors symmetrically, removing only entries in kerby's launcher form or whose resolved path sits under a kerby hook root. Hand-written hook entries with the same script names but different paths are left alone.
