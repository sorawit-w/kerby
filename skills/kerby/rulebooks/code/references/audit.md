...
|---|---|---|
| **Committed secrets** | Reuse the **existing secret-scan regex from the floor's hook** — `<install-root>/rulebooks/base/hooks/pre-commit-check.sh`, where `base` owns the non-disablable scan (code's own `hooks/pre-commit-check.sh` is only a confinement shim into it, with no regex). Grep that file for its `git diff --cached … -G '(…)'` secret pattern and reuse it verbatim — do not retype (it drifts) and do not read code's shim. Run it against the audit scope: working tree and, for `--full`, history via `git log -G`. | `guardrails.md` |
