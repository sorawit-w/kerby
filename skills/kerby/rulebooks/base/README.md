...
|---|---|---|
| `secrets-staged` | data (`gitleaks`/regex floor) | a secret already present in the index or the working tree is hard-blocked when installed, for the commit forms a static pass can resolve (bare, `-C`, `--git-dir`, env selectors, `cd &&`). **Not a proof.** It does not see `git add x && git commit` (the hook runs before staging), nor wrapper-prefixed or aliased invocations, and it deliberately over-blocks rather than parse `git commit`'s arguments — a tripwire on content already present, not a boundary. swe's `threat-model.md` lists every gap and every over-block, with why the precise version was abandoned in each case |
