...
- **Re-grade whenever the work's shape moves, whether or not the line's slots did.** More files, harder logic, a surface you did not expect — any of those can move the grade even when `<files>`, `<change>` and `<check>` still read true. Re-grade first, then re-emit if a slot moved. A re-grade that lands on a different number re-emits the `complexity:` line; one that lands on the same number emits nothing, because the grade already on the record is still the right one. Neither is a stop by itself — what stops work is the bullet below.
- **When an approval you already hold stops covering the work.** The approval stop below is a standing condition — owed whenever the grade is at or above the approval point and no approval covers what you are about to write. This bullet governs something narrower: an approval you were *already given*, and when it stops being good for the work in front of you. Three things end it, each firing once, at the moment it becomes true:

  1. **The grade reaches the approval point while no approval covers the work.** Work that was under the point and now is not needs the stop it did not need before. A task that was approved *at* the point, re-graded below it, and came back unchanged is not this case — the dip never retracted the approval (below), so there is nothing to renew.
  2. **The plan was amended** — the plan the user approved no longer says what you are about to do. Test it against *whatever artifact carried that approval*, which is why this is not a single check:
     ...
     | The full plan block (`workflows/feature.md` § 3) | any of it stopped being true, **its Expected Outcomes included** — a changed predicted response shape, state transition or output is an amendment even when all three slots still read true |
  ...
  3. **The work acquired a property the approval never saw.** A § 3 high-stakes surface it did not already involve, including that section's prose-only traffic-shaping category; a step that cannot be undone; a different approach from the one approved; or a re-grade into the Critical row of the canonical ladder (`workflows/feature.md` § 3), which asks for a staged rollout the approved plan did not describe. Slots can all still read true while any of these is happening, which is why they are named separately: the slots test whether your *declaration* is accurate, not whether the *work* is still the work that was approved.

  A re-grade that changes the number without any of the three is not one of them — do not ask again for a plan nobody amended. Asking repeatedly for the same text is how a reader stops reading it, and the amendment that mattered goes through unread.
...
- **Grade ≥ 7:** after the plan, **STOP and get user approval** before implementing. **This stop is anchored to the grade, not to any artifact.** It fires whenever the grade reaches 7 — including when the full plan block is waived by a user opt-out, and including a grade that only reached 7 partway through the work. `plan_threshold` is capped at 7 so a plan always exists for the approval to read; an opt-out shrinks what the approval reads, never whether approval is taken. Nothing below this line — no route, no escalation, no waiver — removes it.
