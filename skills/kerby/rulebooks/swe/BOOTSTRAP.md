...
- **Re-grade whenever the work's shape moves, whether or not the line's slots did.** More files, harder logic, a surface you did not expect — any of those can move the band even when `<files>`, `<change>` and `<check>` still read true. Re-grade first, then re-emit if a slot moved.
- **Let the new grade decide.** Scope that moved may have moved bands: a grade-6 task that grows into grade 7 needs the approval stop it did not need before, and one that was already ≥ 7 and approved needs *fresh* approval, because the approval was given for what the old plan said. An approval never carries forward across an amendment, and it is never inherited from a grade the task has left. This is the one case where re-emitting is a stop rather than a line.
