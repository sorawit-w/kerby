...
- **Re-emit when the file set or the change itself moves.** If the work turns out to touch a file you did not name, or `<change>` is no longer what you wrote, say so before you touch it — a fresh line. A plan silently outgrowing its own declaration defeats the point of having one.
- **Re-emitting never carries an approval forward.** Where the grade was ≥ 7 and the user approved the work, an amended plan needs fresh approval before you continue: the approval was given for what the old plan said, and scope that grew past it has not been approved by anyone. This is the one case where re-emitting is a stop rather than a line.
