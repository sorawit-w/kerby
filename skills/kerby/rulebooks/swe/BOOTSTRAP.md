...
- **Re-grade whenever the work's shape moves, whether or not the line's slots did.** More files, harder logic, a surface you did not expect — any of those can move the grade even when `<files>`, `<change>` and `<check>` still read true. Re-grade first, then re-emit if a slot moved. A re-grade that lands on a different number re-emits the `complexity:` line; one that lands on the same number emits nothing. Either way it is a line, never by itself a stop — what stops work is the bullet below.
- **When an approval you already hold stops covering the work.** The approval stop below is a standing condition — owed whenever the grade is at or above the approval point and no approval covers what you are about to write. This bullet governs something narrower: an approval you were *already given*, and when it stops being good for the work in front of you. Three things end it, each firing once, at the moment it becomes true:
  ...
  1. **The grade reaches the approval point from below.** Work that was under it and now is not needs the stop it did not need before.
  2. **The plan was amended.** A slot of the `plan:` line stopped being true — the *Re-emit* bullet's test, not the *Re-grade* one directly above — or — where an approved plan-mode plan is carrying the gate and there is no line to amend — the work moved outside what that plan covered. The approval was given for what the old plan said.
