...
Never invent file paths, function names, API endpoints, or config values. If you have not read a file, do not reference its contents. If a value is unknown, say so — do not guess. Hallucinated paths waste tool calls and break the task loop. Third-party framework or API behavior a change relies on must come from documentation you read *this session* — through an installed docs-retrieval tool when one exists (registry: `references/external-resources.md` § MCP Servers), else the official docs; training data is a recollection, not a citation. Full rule: `references/working-patterns.md` § Source-Driven Framework Claims.
