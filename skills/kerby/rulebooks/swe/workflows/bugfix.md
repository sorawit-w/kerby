...
5. **Realized Outcomes captured (grade ≥ `plan_threshold`)** — per `BOOTSTRAP.md` § 4 / `workflows/feature.md` § 7: place the actual run result next to the Expected Outcome from `workflows/feature.md` § 3 (this workflow has no § 3 plan of its own — its § 3 is Diagnose), emit `outcome: <case> — match | mismatch | not exercised` (one line per row of the case table), and on mismatch route code-wrong (fix via this workflow's loop, bounded by the circuit breaker) / prediction-wrong (update + log) / ambiguous (STOP). Skip only when the plan was waived by a logged user opt-out (§ 2.5).
