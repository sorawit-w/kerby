...
The happy path is always a row. Edge and failure are each required where the change has one; where a category has none, write one row for it whose `case` is `none — <why>` (a pure rename has no failure path; say so in the row). This is a forced artifact, not advice: an absent table is a missing plan, not a stylistic choice. Each row is a check the finish step (§ 7 step 6) runs for real and fills an `actual` column for. A row whose condition cannot be exercised — a `none` row, or a failure whose trigger never arose — records `not exercised` in `actual` and `outcome: <case> — not exercised`, never `match`: a match is a comparison, and a case that never ran has nothing to compare. At the High/Critical tiers the table is also the QA sub-agent's Stage 1 checklist (`references/validation.md`).
...
6. **Realized Outcomes captured (grade ≥ `plan_threshold`)** — distinct from "How to Verify" above (that's instructions for the human; this is *your* check against the § 3 prediction). *Skip this step only when the plan was waived by a logged user opt-out (`BOOTSTRAP.md` § 2.5) — there is no Expected Outcome to compare against; standard Verification (§ 6) still applies.* After implementing:
   1. Capture the **actual** result from a real run — or a dry-run transcript where no runnable surface exists — and place it next to the § 3 Expected Outcome. Evidence is an object (screenshot path / captured JSON / CLI dump / diff), **not** prose. Fill the case table's `actual` column row by row from real runs — one evidence object per row, or `not exercised` for a row whose condition never arose. A plan owed at this grade that has no case table is a plan defect: add the table now (§ 3), then run it — never fall back to a table-less comparison.
   2. Emit one line per case: `outcome: <case> — match | mismatch | not exercised`.
