...
| Commit secrets | `pre-commit-check.sh` (PreToolUse Bash `git commit`) hard-block — betterleaks or gitleaks if present (via stable `stdin` mode), else built-in regex | `[enforced-when-installed]` | Scans **added** lines in the working tree + index diff against `HEAD`, not history; regex fallback is a narrow floor; an external scanner uses the target repo's own allowlist. Command recognition is deliberately narrow — see **Secret scan: what it sees** below |
...
**It scans against `HEAD`, not the index.** `git commit -a`, `git commit <path>` and `--include` commit **working-tree** content, so a `--cached` scan did not see the bytes those forms actually commit. Diffing `HEAD` covers staged *and* modified-tracked content in one pass — and needs no argument parsing to decide which, which is the whole point.
