...
| Commit secrets | `pre-commit-check.sh` (PreToolUse Bash `git commit`) hard-block — betterleaks or gitleaks if present (via stable `stdin` mode), else built-in regex | `[enforced-when-installed]` | Scans staged added lines only, not history; regex fallback is a narrow floor; an external scanner respects its own repo-local allowlist. **Command recognition shares protect-git's limits** (matcher parity-tested): it resolves bare, `-C`, `--git-dir`, `GIT_DIR`/`GIT_WORK_TREE`/`GIT_INDEX_FILE` env selectors, and `cd &&` forms, scanning the repo *each* one targets, and ignores `git commit` appearing inside a quoted argument. **Two gaps are structural to the mechanism, not bugs to be patched, and the first is common:** (a) the hook fires *before* the shell runs, so anything staged by the same command — `git add x && git commit` — is not in the index yet and is never scanned; (b) a `git` alias (`git ci`) resolves at runtime and is invisible to a string parser. Also unresolvable: a commit reached only through a pipe or subshell, `cd -`/bare `cd`, cumulative *relative* `-C`/`--git-dir`, or a quoted path/separator containing spaces. **Treat this as a tripwire on already-staged content, not a boundary** — the mechanism that would close (a) and (b) is a repo-side `pre-commit` hook running at commit time against the real index, a second install model kerby has so far declined. Until v9.15 the matcher was a literal leading `git commit`, so every non-bare form skipped the scan entirely (issue #46); the v9.16 fix added the forms above and corrected two regressions it briefly introduced |
