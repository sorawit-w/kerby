...
|---|---|---|---|
| Edit a populated `.env` | `protect-env.sh` (PreToolUse Edit\|Write) hard-block | `[enforced-when-installed]` | Edit/Write tool only — a shell `printf … > .env` is not seen. Scope is deliberately narrower than all `.env*`: templates (`.env.example`/`.template`/`.sample`) and creating an absent `.env` are allowed, because neither can destroy credentials and the commit scan already covers a secret pasted into a committed template. See `references/guardrails.md` § Environment Files |
