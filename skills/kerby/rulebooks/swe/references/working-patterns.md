...
- *"I'll clean this up in a follow-up PR."* → An **unrecorded** follow-up is the excuse; the promise is the whole problem, because nothing outside your head knows the work exists. A deferral written down before you finish — a `## Follow-ups` line in `ROADMAP.md`, per `references/roadmap.md` — is not this excuse, it is the discipline that replaces it. Deferring is a decision you are allowed to make and required to record. What you may never defer is on the never-defer list in `BOOTSTRAP.md` § 1b.
