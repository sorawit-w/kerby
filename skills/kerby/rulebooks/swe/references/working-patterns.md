...
This is especially common when adopting `kerby` into a project that predates the test-first discipline — characterization tests are the bridge into the change-comfort regime the rest of these rules assume.
...
- **Prefer boring, readable solutions** over clever abstractions — and don't add abstraction a single use doesn't earn: no interface with one implementation, no factory for one product, no config knob for a value that never changes. Inline it until a second caller forces the seam.
- **Hoist repeated literals to a single named source** — the flip side of the rule above: once a literal (especially a *set of allowed values* — a status list, role set, or string/number array) is referenced at ≥2 real call sites, *or* would force edits in multiple places if it changed, the "second caller" has arrived. Stop re-typing the literal — define it once as a named, exported constant and import it everywhere. Co-locate it with its primary use; promote to a shared module only when it's consumed across modules. In TypeScript, derive the type *from* the constant rather than hand-maintaining a parallel union — `export const ROLES = ['admin', 'editor', 'viewer'] as const; export type Role = typeof ROLES[number];` — so the value and its type can't drift. This is a **deduplication** move (it kills drift and restores IDE find-usages, which a bare literal doesn't have), *not* externalization: the constant stays in code. Externalize to config/env only when the value independently meets the environment-varies-or-gates-risk trigger in `validation.md`; secrets stay in `.env` per `guardrails.md`. Single-use literals stay inline — speculative extraction is its own smell.
...
- *"I'll clean this up in a follow-up PR."* → An **unrecorded** follow-up is the excuse; the promise is the whole problem, because nothing outside your head knows the work exists. A deferral written down before you finish — in the deferral sink, per `references/guardrails.md` § Where a finding goes — is not this excuse, it is the discipline that replaces it. Deferring is a decision you are allowed to make and required to record. What you may never defer is on the never-defer list in `BOOTSTRAP.md` § 1b.
