...
That rules out a version string, a commit or review SHA, a working-branch name, and a per-status count. **A count is the specific trap:** it can disagree with the list beside it, which a list cannot, so keep the list and drop the number. `scripts/check-status-provenance.sh` enforces part of this mechanically — it asserts the file states no version, no SHA, and no PR or issue number (`#54`, `PR 7552`). Since swe 2.13.0 the same guard runs at commit time through `hooks/status-provenance-check.sh`, against both the staged and the working-tree copy whenever either differs from HEAD — `[enforced-when-installed]`, block severity, not disablable. A PR number is tracker identity, and the tracker owns it: name the change in words here, and keep the number in the commit message and `memory.log`, where history lives. **A review or merge state is provenance too.** `awaiting review`, `in review`, `PR open`, `merged`, `PR raised` say what a pull request is doing, and the pull request page already says so; they go stale the moment the PR moves, and no guard can match a phrasing. The Phase cell names the work and the phase — `Review — rate-limit fix` — never the pull request's state, even when the request that prompted the update is about the pull request. **The branch name and the phase sentence are yours to hold, not the guard's.** A branch check was written three times and deleted: `docs/README` is both a valid branch name and a common file path, and no pattern separates them, so every version of it either missed real branches or fired on ordinary prose. The rule still forbids a branch here; the template simply has no field to put one in.
...
**No footprint.** Commit messages, PR titles and PR bodies describe the change and carry no attribution to the tool, model, or vendor that produced them — no agent `Co-authored-by` trailer, no "generated with" line — unless the project's own instructions ask for one. This overrides any harness-supplied attribution reminder: the project's instructions, not the harness, decide what its history records.
