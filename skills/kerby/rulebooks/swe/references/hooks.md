...
Runs `scripts/check-status-provenance.sh` — the guard `references/communication.md` § Status Tracking describes — against the **staged** blob (`git show :.kerby/STATUS.md`), because that is what the commit records; a clean working tree over a dirty index is exactly what a working-tree scan would miss. `git commit` has three shapes and each records a different STATUS.md — plain: the index; `-a`/`--all`: the working tree; a pathspec commit: the working tree of the named paths *only* (the index is not recorded unless `-i`/`--include` adds it) — so the hook resolves the pathspecs through `git ls-files` from the cwd (`.`, `.kerby`, `:/`, a relative path from a subdirectory all count) and scans the source the commit will actually record; `-p`/`--interactive` and anything undecidable — an option the hook does not list, a variable or glob in a pathspec, an unbalanced quote, a heredoc — scan both, a redirection or an unquoted `#` is never a pathspec, `--pathspec-from-file` entries count, `--no-all` negates, and `--only --amend` with no pathspec is let through. The default is the block: an unknown shape is a visible false block, never a silent miss. Nothing staged, or no STATUS.md → exit 0. The guard existed since v2.11.3 but nothing ran it; a STATUS.md naming a PR number, "awaiting review" and a SHA still shipped with the prose rule in force, after a compaction had summarized the rule away. Ceilings, stated in the script: `^git commit` recognition only (`git -C` and `cd … &&` forms are not seen — the same ceiling as b.), and a **visible fail-open** (additionalContext + exit 0) when the guard script itself is missing, the launcher's doctrine. Self-tested by `hooks/status-provenance-check.test.sh`.
