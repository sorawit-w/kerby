...
| Change touches a third-party framework/API surface (new dependency, SDK call, framework lifecycle/hook, library config) | MCP | Docs-retrieval MCP (e.g., context7) | No docs-retrieval tool is installed | `BOOTSTRAP.md` § Accuracy requires third-party behavior to come from docs read this session; retrieval makes that cheap and version-exact |
