...
Current release: `9.26.3` — the status file holds position, never provenance. `.kerby/STATUS.md` shipped naming a version two patches behind every surface the release actually lives in, a stale commit SHA, an old timestamp and a row counting a PR that had already merged — all four wrong when written, and no gate reads that file. Versions live in the manifests, commits and branches in git, history in `memory.log`; a copy one tier below its authority can only drift, so every restated field is deleted rather than corrected. Deletion is what makes the rule checkable: a new guard asserts an *absence* — no semver, no SHA-shaped hex token — and it ships with its test. It guards two things rather than three: a branch check was written three times and deleted, because `docs/README` is both a valid branch name and a common file path and no pattern separates them. What it cannot catch is written down rather than papered over: branch names, a wrong phase sentence, a SHA abbreviation that is all letters or all digits. Previously: `9.26.2` — one more site stops picking a deferral sink for itself. See [CHANGELOG.md](CHANGELOG.md) for the full history.
