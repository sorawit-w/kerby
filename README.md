...
Current release: `10.2.0` — swe 2.13.0: the status file is checked at commit time, and the history carries no footprint. The provenance guard gains two token shapes (`#54`, `PR 7552`) and runs as a block-severity commit hook on the STATUS.md the commit records; § Status Tracking names the review-state phrasings a token guard cannot match; commit messages and PR bodies carry no tool, model or vendor attribution unless the project asks for one. Previously: `10.1.0` — the rules come back after a compaction. `kerby load` puts the rules into context as a tool result, and compaction summarizes tool results; now the SessionStart hook, when registered, re-injects every pinned builtin's eager prose on `source: "compact"`, derived from each manifest and read only from the install root, so an agent never has to notice the rules are gone. `reload` remains the manual path for externals and for harnesses without SessionStart. Previously: `10.0.0` — the `codex-review` rulebook is retired. A review-until-clean loop over a model review has no stable result; the GitHub-side `@codex review` found new defects after clean local passes anyway, and a capped review → fix → re-review sequence is a workflow, not a rulebook. Three builtins ship; the PR review is the one path; the v9.0.0 `code` → `swe` migration residue is gone on schedule. Previously: `9.28.0` — swe 2.12.0: a case table in every threshold-gated plan, and docs you read this session. See [CHANGELOG.md](CHANGELOG.md) for the full history.
