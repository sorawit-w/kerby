...
Current release: `9.21.0` — a killed review can no longer be marked clean. `codex-mark` took the last `CODEX_VERDICT:` line in a transcript, which is only the run's own conclusion if the run finished. A stalled review left a log holding six verdict lines quoted from *earlier* reviews it had read as evidence — the last being another PR's clean `P0=0 P1=0`. Marking it would have passed a review that never reached a verdict. The fix keeps the evidence out of band — a first attempt used a sentinel line *inside* the transcript and had the same flaw, since a reviewer reading the diff quotes it. `codex-run` now writes a sidecar on its clean exit path only, recording the transcript's length and a digest of exactly those bytes; `codex-mark` verifies both before parsing, so a truncated or substituted transcript is refused rather than silently reparsed. Previously: `9.20.0` — the locator stops letting the workspace choose the gate. See [CHANGELOG.md](CHANGELOG.md) for the full history.
