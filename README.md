...
Current release: `9.17.0` — the env guard learns which file it was actually protecting. `protect-env` used to block every path matching `.env`, template files included, so an agent could not maintain the `.env.example` that tells a developer which variables a repo needs — while the commit-time secret scan, which is filename-agnostic, already caught a real key pasted into one. The guard now covers what only it can see: a real `.env` is gitignored, never staged, and has no undo. Templates are editable, creating an absent `.env` is allowed, and the moment that file has content the door shuts again. No override token — the escape stays a deliberate settings edit. `protect-env` also picked up the self-test it had been shipping without. Previously: `9.16.0` — the secret scan gets a door git opens. The floor has always run *before* a Bash command, reading the command text; sixteen review rounds made that reading correct, and it still could not see `git add x && git commit`, because nothing is staged until after the check has passed — the most common commit shape an agent produces. `install` now offers a git `pre-commit` hook: same scanner, second front door, and git hands it the real index instead of a command to guess from. It is opt-in per repo, index-only, refuses to write a file `core.hooksPath` would shadow, and fails open rather than wedging a repo if the install moves. `git commit --no-verify` still skips it — which is exactly why the pre-command check stays. — see [CHANGELOG.md](CHANGELOG.md) for the full history.
