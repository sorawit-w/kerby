...
Current release: `9.15.0` — swe 2.6.0 settles a rule that argued with itself. The gate-tier table exempted docs from the full gate; twelve lines later the commit-time rule revoked the exemption for every commit — and five other files restated the absolute independently, six counting the canonical one's own contradiction. An agent reading any pair ran build+lint+test on a typo fix and learned that the tiers are decoration. The tier is now chosen from the staged diff by one canonical section every other file defers to, *docs-only* means no file any gate reads (a version string a check parses is a gate input wearing a `.md` extension), and a parity guard plus its test keep the rule from drifting back into a second file. Riding along: squash-merge is scoped to short-lived branches (squashing between two long-lived ones records no ancestry, so the next merge conflicts on files nobody touched), and exact-decimal domains — money, tax, billing — never use binary floating point. — see [CHANGELOG.md](CHANGELOG.md) for the full history.
