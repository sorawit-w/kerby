...
Current release: `9.26.3` — the status file holds position, never provenance. `.kerby/STATUS.md` shipped naming a version two patches behind what six manifests said, a stale commit SHA, an old timestamp and a row counting a PR that had already merged — all four wrong when written, and no gate reads that file. Versions live in the manifests, commits and branches in git, history in `memory.log`; a copy one tier below its authority can only drift, so every restated field is deleted rather than corrected. Deletion is what makes the rule checkable: a new guard asserts an *absence* — no semver, no SHA, no branch name — which matches shapes rather than phrasings, and it ships with its test and a false-positive probe. It cannot catch a wrong phase sentence; that limit is stated, not papered over. Previously: `9.26.2` — one more site stops picking a deferral sink for itself. See [CHANGELOG.md](CHANGELOG.md) for the full history.
