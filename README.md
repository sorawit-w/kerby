...
Current release: `9.23.0` — hooks get tiers, and a guard that proves them. Which hooks an env var may switch off was a hand-written table; it fell behind every time the hook set moved, and had quietly dropped one hook. The answer was already in the manifests — `floor` marks what nothing may loosen, `severity` says whether a finding denies or holds — so the table became a rule: a hook honors the disable list if and only if its tier is `optional`. A guard checks the mechanical half of that across every shipped script — whether each one declares the canonical disable block its tier calls for — and treats anything it cannot classify as an error, not a pass. It does not watch the hooks run, and says so. Also: the `floor` contract row stops contradicting the validator, `override` is documented as the inert field it always was, and `status` no longer guesses that an unregistered hook was one you declined. Previously: `9.22.0` — a question stops being treated as a task. Ask an agent why some code behaves the way it does and it would run the test suite, commit, and write `.kerby/memory.log` — none of it requested, and none of it improvised either. The routing table held seven rows and every one was a change task, so "explain this" matched none and fell to the nearest, `quick-task`, whose steps run the gate and commit without ever asking whether anything changed. `investigate` is now the first row and the first value in the route enum agents already emit. It reaches no workflow at all: no writes, no branch, no commit, and no gate run to certify the answer — though running a command to *observe* is still fine and often the whole job. The evidence for an answer is the file:line citation the diagnosis rule already demands, not a green suite. Previously: `9.21.0` — a killed review can no longer be marked clean. See [CHANGELOG.md](CHANGELOG.md) for the full history.
