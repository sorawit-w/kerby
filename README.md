...
Current release: `9.16.0` — the secret scan gets a door git opens. The floor has always run *before* a Bash command, reading the command text; sixteen review rounds made that reading correct, and it still could not see `git add x && git commit`, because nothing is staged until after the check has passed — the most common commit shape an agent produces. `install` now offers a git `pre-commit` hook: same scanner, second front door, and git hands it the real index instead of a command to guess from. It is opt-in per repo, index-only, refuses to write a file `core.hooksPath` would shadow, and fails open rather than wedging a repo if the install moves. `git commit --no-verify` still skips it — which is exactly why the pre-command check stays. — see [CHANGELOG.md](CHANGELOG.md) for the full history.
