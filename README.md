...
Current release: `9.26.0` — a plan before every change, and the smallest change that still finishes the job. Below grade four an agent could start editing with nothing stated, so the developer learned what changed by reading the transcript afterwards. The Plan Gate now decides how big the plan is, never whether there is one: exactly one of three artifacts — a one-line floor, the full plan block, or quick-task's fit check — each naming the files, the change and the check, emitted before the first edit that implements the task rather than at the grade, where a file list would still be a guess. `just do it` shrinks that plan; it no longer removes it — an exception to the priority order that the rules declare rather than hide. Alongside it, minimal is not partial: the change ships everything that was asked and defers only hardening, refactors, cleanup and polish, each one recorded rather than promised — in `ROADMAP.md` § Follow-ups, the project's tracker, or the session log, whichever the repo actually has. Previously: `9.25.1` — Copilot invokes kerby's hooks, and on Windows that is worse than not invoking them. See [CHANGELOG.md](CHANGELOG.md) for the full history.
