...
Current release: `4.22.0` — gives `cerby audit` a dedicated render template and
pins its Markdown body structure, so the same findings always produce a
structurally identical report (determinism is the acceptance bar). See [CHANGELOG.md](CHANGELOG.md).
