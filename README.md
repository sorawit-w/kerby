...
Current release: `9.26.2` — one more site stops picking a deferral sink for itself. One place is meant to decide where deferred work is recorded, with every other mention carrying a pointer rather than a copy so there is nothing to drift — the intent, not yet the state of the corpus. A line in the vendor-adapter doctrine still named `ROADMAP.md` outright, which points at a file that does not exist in a repo using an external tracker. What it was really saying survives untouched: a vendor migration is scheduled work, never a side effect of touching a legacy file for some other reason. Three further sites still choose for themselves and are logged, not fixed — including a second one in the same file. Two of them say "ticket" and one says "roadmap item", which is how a sweep for filenames missed all three. Previously: `9.26.1` — an approval you already hold, and when it stops covering the work. See [CHANGELOG.md](CHANGELOG.md) for the full history.
