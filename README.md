...
Current release: `9.26.1` — an approval you already hold, and when it stops covering the work. The previous release asked the user to re-approve identical text every time a grade-7 task merely looked harder; a reader asked to approve the same paragraph three times stops reading it, which is how the amendment that mattered goes through unread. The approval stop itself is untouched — still a standing condition, owed whenever the grade is at the approval point and no approval yet covers the work, including under an opt-out. What narrowed is its *renewal*: an approval already given ends when the plan is amended, or when the work acquires a property the approval never saw — a high-stakes surface, an irreversible step, a swapped approach. Arriving at the point without an approval was never the same question. A re-grade that moves the number and nothing else is a line, not a stop. Previously: `9.26.0` — a plan before every change, and the smallest change that still finishes the job. See [CHANGELOG.md](CHANGELOG.md) for the full history.
