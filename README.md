...
Current release: `9.26.0` — a plan before every change, and the smallest change that still finishes the job. Below grade four an agent could start editing with nothing stated, so the developer learned what changed by reading the transcript afterwards. The Plan Gate now decides how big the plan is, never whether there is one: the floor is one line naming the files, the change and the check, emitted immediately before the first edit rather than at the grade, where a file list would still be a guess. `just do it` shrinks that plan; it no longer removes it — the one place this playbook outranks an explicit instruction, and it says so. Alongside it, minimal is not partial: the change ships everything that was asked and defers only hardening, refactors, cleanup and polish, each one recorded in `ROADMAP.md` § Follow-ups rather than promised. Previously: `9.25.1` — Copilot invokes kerby's hooks, and on Windows that is worse than not invoking them. See [CHANGELOG.md](CHANGELOG.md) for the full history.
