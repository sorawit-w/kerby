...
Current release: `9.17.0` — the env guard learns which file it was actually protecting. `protect-env` used to block every path matching `.env`, template files included, so an agent could not maintain the `.env.example` that tells a developer which variables a repo needs — while the commit-time secret scan, being filename-agnostic, already treated a key pasted into one exactly as it treats a key in `config.ts` (coverage that is only as strong as the scanner you have installed, not a guarantee). The guard now covers what only it can see: a real `.env` is gitignored, never staged, and has no undo. Templates are editable, creating an absent env file is allowed, and the moment that file exists — empty counts — the door shuts again. The allow-list is on names but the decision is on objects: relative paths, symlinks, hard-linked templates, non-regular files, and any spelling the filesystem folds onto a differently-named entry all block. No override token — the escape stays a deliberate settings edit. `protect-env` also picked up the self-test it had been shipping without. Previously: `9.16.0` — the secret scan gets a door git opens.
