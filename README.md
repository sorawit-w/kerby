...
/kerby reload      # re-load by hand after a compaction (the SessionStart hook does it when registered)
...
Current release: `10.1.0` — the rules come back after a compaction. `kerby load` puts the rules into context as a tool result, and compaction summarizes tool results; now the SessionStart hook, when registered, re-injects every pinned builtin's eager prose on `source: "compact"`, derived from each manifest and read only from the install root, so an agent never has to notice the rules are gone. `reload` remains the manual path for externals and for harnesses without SessionStart. Previously: `10.0.0` — the `codex-review` rulebook is retired. A review-until-clean loop over a model review has no stable result; the GitHub-side `@codex review` found new defects after clean local passes anyway, and a capped review → fix → re-review sequence is a workflow, not a rulebook. Three builtins ship; the PR review is the one path; the v9.0.0 `code` → `swe` migration residue is gone on schedule. Previously: `9.28.0` — swe 2.12.0: a case table in every threshold-gated plan, and docs you read this session. Expected Outcomes gains a tester's case table (happy path, edge, failure) that the finish step fills with an `actual` column and judges per case, and that the QA sub-agent uses as its Stage 1 checklist; § Accuracy now requires third-party framework and API behavior to come from something read this session — the documentation through a docs-retrieval tool, else the official docs, else the installed package's own source or type definitions — with a docs-retrieval MCP (context7) in the registry. Previously: `9.27.0` — hook registration survives an install that moves. See [CHANGELOG.md](CHANGELOG.md) for the full history.
