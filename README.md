...
Current release: `9.20.0` — the locator stops letting the workspace choose the gate. The install root was found with a bare recursive glob run from the working directory, and that root decides which rulebook content is install-trusted — so any vendored or forked copy of kerby inside the tree could win on first-match. In kerby's own repo it did. The order is now the harness-provided skill directory, then `KERBY_DIR`, then ask — deliberately no rung that searches, because a project-local path is workspace content and an enumerated list cannot be kept complete. A builtin pin with a stale path is now repaired from the install rather than failing closed: the id is the whole trust test, and no other pin field takes part in it. Previously: `9.19.0` — state lands inside the PR that produced it. See [CHANGELOG.md](CHANGELOG.md) for the full history.
